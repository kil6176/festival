package com.spring.festival.common;
...
public class PagingUtil {
	...
	public static CommonDTO setPageUtil(CommonDTO commonForm) {
		...
		if(fc_num != 0)
		{
			...
			if(totalPageCount > 5)
				pagination += "<a href='javascript:" + functionName + "(\"" + fc_num +","+ totalFirstPage + "\");' class=\"direction_left01\">[<<]</a>";
			
			if(totalPageCount > 1)
				pagination += "<a href='javascript:" + functionName + "(" + fc_num +"," + prePerPage + ");' class=\"direction_left01\">[<]</a>";
			
			for (int a = viewFirstPage; a <= ViewLastPage; a++) {
				if (a == currentPage) {
					pagination += "<a href='javascript:" + functionName + "(" + fc_num +"," + a + ");' class='onpage'>[" + a + "]</a>";
				} else {
					pagination += "<a href='javascript:" + functionName + "(" + fc_num +",\"" + a + "\");'>[" + a + "]</a>";
				}
			}
			
			if(totalPageCount > 5)
				pagination += "<a href='javascript:" + functionName + "(" + fc_num + "," + nextPerPage + ");' class=\"direction_right01\">[>]</a>";
		...
		System.out.println("countPerList"+countPerList);
		System.out.println("countPerPage"+countPerPage);
		System.out.println("totalPageCount"+totalPageCount);
		System.out.println("fc_num"+fc_num);
		return commonDto;
	}
}
