...
<script type="text/javascript">
	...
	function goBoardList(){				
		location.href = "/fb_board/boardList.do";
	}
	...
	function goBoardUpdate(){
		...
		location.href = "/fb_board/boardUpdate.do?board_num="+ board_num;
	}
	
	/** 게시판 - 첨부파일 다운로드 */
	function fileDownload(bf_name_key, bf_name, bf_path){
			
		location.href = "/fb_board/fileDownload.do?bf_name_key="+bf_name_key+"&bf_name="+bf_name+"&bf_path="+bf_path;
	}
	...
	function getBoardDetail(board_num){
		...
		if(board_num != ""){
			...
			$.ajax({	
			    ...
			    url		: "/fb_board/getBoardDetail.do",
		...
		if(obj != null){								
			...
			if(filesLen > 0){
				...
				for(var a=0; a<filesLen; a++){
					...
					str += "<td colspan='3'><a href='/fb_board/fileDownload.do?bf_name_key="+encodeURI(bf_name_key)+"&bf_name="+encodeURI(bf_name)+"&bf_path="+encodeURI(bf_path)+"'>" + bf_name + "</a></td>";
					str += "</tr>";
				}	
			}			
