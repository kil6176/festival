...
<script type="text/javascript">
	...
	function goBoardUpdate(){
		...
		location.href = "/board/boardUpdate?board_num="+ board_num;
	}
	
	/** 게시판 - 첨부파일 다운로드 */
	function fileDownload(bf_name_key, bf_name, bf_path){
			
		location.href = "/board/fileDownload?bf_name_key="+bf_name_key+"&bf_name="+bf_name+"&bf_path="+bf_path;
	}
		...
		if(obj != null){								
			...
			if(filesLen > 0){
				...
				for(var a=0; a<filesLen; a++){
					...
					str += "<td colspan='3'><a href='/board/fileDownload?bf_name_key="+encodeURI(bf_name_key)+"&bf_name="+encodeURI(bf_name)+"&bf_path="+encodeURI(bf_path)+"'>" + bf_name + "</a></td>";
					str += "</tr>";
				}	
			}			
